"""0027 · partial derivatives and gradients — full solution.

Run:  python3 practice/solutions/0027-partial-gradients.py
...
def directional_derivative(f, x, d, h=1e-6):
    ...
    u = [v / n for v in d]
    g = gradient(f, x, h)
    return sum(a * b for a, b in zip(g, u))
...
def descent_step(f, x, eta, h=1e-6):
    ...
    g = gradient(f, x, h)
    return [xi - eta * gi for xi, gi in zip(x, g)]
