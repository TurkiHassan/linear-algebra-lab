"""0028 · Jacobians and vector-valued gradients — full solution.

Run:  python3 practice/solutions/0028-jacobian.py
...
def jacobian_shape(J):
    ...
    return (len(J), len(J[0]) if J else 0)
...
def loss_gradient(Phi, y, theta):
    ...
    e = residual(Phi, y, theta)
    D = len(theta)
    return [-2 * sum(e[i] * Phi[i][j] for i in range(len(e))) for j in range(D)]


def descent_step(Phi, y, theta, eta):
    """One gradient descent update on the parameters."""
    g = loss_gradient(Phi, y, theta)
    return [t - eta * gi for t, gi in zip(theta, g)]
